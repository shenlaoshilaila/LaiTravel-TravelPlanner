...
// Friendlier minute floor for tiny hops
...
async function fetchSegInfo(p1: POI, p2: POI): Promise<SegInfo> {
  // If they're effectively the same location, short-circuit.
  if (isSameSpot(p1, p2)) {
    return { durationText: "0 min", distanceText: "0.0 km" };
  }

  // Mock mode (no Google call)
  if (MODE === "mock") {
  ...
  try {
    const url = MODE === "nextapi" ? "/api/distance" : "http://localhost:8080/api/distance";
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      // credentials: "include", // uncomment if your Spring uses cookies
    ...
    // Fallbacks to mock values if any piece is missing
    ...
    // Network or parsing error → mock fallback
    const mins = haversineMinutes(p1, p2);
    const km = haversineKm(p1, p2);
    return { durationText: `${mins} min drive`, distanceText: `${km.toFixed(1)} km` };
  }
...
export default function DayPOISection({
  ...
  // Controlled list (parent owns actual state)
  const pois = initialPois;

  // Cache segment info (duration + distance)
  const [segInfoMap, setSegInfoMap] = useState<Record<string, SegInfo>>({});
  const [loadingKeys, setLoadingKeys] = useState<Record<string, boolean>>({});

  // Build segments, skipping same-spot pairs right here
  const segments = useMemo(() => {
    const arr: Array<{ from: POI; to: POI; k: string }> = [];
    for (let i = 0; i < pois.length - 1; i++) {
      const from = pois[i];
      const to = pois[i + 1];
      if (isSameSpot(from, to)) continue; // skip zero-length segment
      arr.push({ from, to, k: segKey(from, to) });
    }
    return arr;
  }, [pois]);

  // Fetch seg info for any missing segment keys
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      for (const s of segments) {
        ...
        const info = await fetchSegInfo(s.from, s.to);
        if (cancelled) return;
  ...
  }, [segments.length, segments.map((s) => s.k).join("|")]);
                ...
                <React.Fragment key={`day-${day}-seq-${p.sequence}`}>
                  ...
                  {hasNext && (
                      <li className="ml-2 list-none flex items-center text-blue-600 text-sm">
                        ...
                        {/* If the next POI is basically the same spot, show a friendly message */}
